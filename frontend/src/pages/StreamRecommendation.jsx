...
import { BookOpen, ArrowRight, CheckCircle2, FlaskConical, GraduationCap } from 'lucide-react';
...
function StreamRecommendation() {
    ...
    // Safely extract fields — they can be strings OR arrays depending on backend version
    ...
    // Normalize to arrays
    const courseDegrees = Array.isArray(courseDegreeRaw)
        ? courseDegreeRaw
        : courseDegreeRaw ? [courseDegreeRaw] : [];
    const academicStrengths = Array.isArray(academicStrengthRaw)
        ? academicStrengthRaw
        : academicStrengthRaw ? [academicStrengthRaw] : [];
        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', alignItems: 'center' }}>
                {['Scoring', 'Career Match', 'Stream', 'Roadmap', 'Dashboard'].map((step, i) => (
                    <div key={step} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <div style={{
                            background: i < 2 ? 'rgba(59,130,246,0.3)' : i === 2 ? '#8B5CF6' : 'rgba(255,255,255,0.1)',
            ...
            <div className="glass-card animate-fade-in" style={{ width: '100%', maxWidth: '700px', padding: '2.5rem' }}>
                ...
                <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>
                    Based on your career match and profile, our engine recommends the following academic path for{' '}
                    <strong style={{ color: '#fff' }}>{topMatch?.role || 'your career'}</strong>.
                </p>

                {/* Recommended Stream */}
                <div style={{
                    background: `linear-gradient(135deg, ${streamColor}22, ${streamColor}0a)`,
                    border: `1px solid ${streamColor}44`,
                ...
                {courseDegrees.length > 0 && (
                    <div style={{ marginBottom: '1.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                            ...
                            <h4 style={{ color: 'var(--text-muted)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>Recommended Course / Degree</h4>
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                            {courseDegrees.map((course, i) => (
                                <span key={i} style={{
                                    background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.3)',
                                    color: '#c4b5fd', padding: '0.4rem 1rem', borderRadius: '999px', fontSize: '0.9rem', fontWeight: 500
                                }}>
                                    {course}
                                </span>
                            ))}
